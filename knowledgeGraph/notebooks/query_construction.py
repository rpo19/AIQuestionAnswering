...
{self.exclusions}
    ...
    def __get_candidate_entities(self, cn, relation, endpoint = 'http://dbpedia.org/sparql'):
        ...
        if relation.direction == 'outgoing':
            ...
            q = f"SELECT DISTINCT ?obj WHERE {{ {body} }}"
        else:
            for entity in cn:
                if not first: 
                    body += "UNION "
            ...
            q = f"SELECT DISTINCT ?sbj WHERE {{ {body} }}"
...
if __name__ == "__main__":
    ...
    Q = query_builder.build(question='Who is the wife of Barack Obama?', entities=["http://dbpedia.org/resource/Barack_Obama"], pattern='p1')
